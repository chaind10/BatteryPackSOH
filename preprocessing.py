...
df_dataset = pd.read_excel(file_path, sheet_name='Data')

# Drop unnecessary columns
df_dataset.drop(columns=['Mat', 'No.', 'ID'], inplace=True)

# Remove missing values
df_dataset.dropna(inplace=True)

# Ensure all numeric columns are numeric
df_dataset = df_dataset.apply(pd.to_numeric, errors='ignore')

# Sort by 'Q' (charge quantity)
sortedByQ = df_dataset.sort_values(by='Q')

# Sort by 'SOC' (state of charge)
sortedBySOC = df_dataset.sort_values(by='SOC')
...
df_dataset['AvgCellVoltage'] = df_dataset[u_cols].mean(axis=1) #calculating average cell voltage for each row
